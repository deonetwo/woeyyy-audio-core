@echo off
setlocal
title Woeyyy Pro Audio Launcher - Mic Boost, Voice Changer & Soundboard
cd /d "%~dp0"

echo ============================================================
echo    Woeyyy Pro Audio - Microphone Enhancer, Voice Changer & Soundboard
echo ============================================================
...
call .venv\Scripts\activate.bat
echo [*] Installing audio engine packages from requirements.txt...
...
:launch
echo [*] Launching Woeyyy GUI...
python gui.py

:: If the app exited with an error, keep console open for troubleshooting
if errorlevel 1 (
    echo.
    echo [NOTICE] Woeyyy exited with an error. See details above.
    pause
)
